...
import {
  isDeepSubdomain,
  isHostnameWithinNamespace,
  normalizeGrantSet,
  normalizeHostname,
  normalizeNamespaceGrant,
  resolveRelativeOwner,
} from "../../app/lib/shared/dns/hostname";
import {
  assertHostnameAccess,
  assertRecordAccess,
  canSeeRecord,
} from "../../app/lib/server/permissions/dns-authorization.server";
...
describe("DNS label boundary authorization", () => {
  ...
  it.each([
    "nycu.club",
    "evilmagic.nycu.club",
    "magic.nycu.club.evil.example",
    "photo.nycu.club",
    "magic..nycu.club",
    "foo*magic.nycu.club",
    "%2e.magic.nycu.club",
    "https://magic.nycu.club",
  ])("rejects %s", (hostname) => {
  ...
  it("allows valid underscore and wildcard labels only when explicitly enabled", () => {
    expect(normalizeHostname("_acme-challenge.magic.nycu.club", { allowUnderscore: true }))
      .toBe("_acme-challenge.magic.nycu.club");
    expect(normalizeHostname("*.magic.nycu.club", { allowWildcard: true }))
      .toBe("*.magic.nycu.club");
    expect(() => normalizeHostname("_acme.magic.nycu.club")).toThrow(AppError);
    expect(() => normalizeHostname("foo.*.magic.nycu.club", { allowWildcard: true }))
      .toThrow(/最左側/u);
  });
  ...
  it("enforces label and FQDN length limits", () => {
    ...
    expect(normalizeHostname(`${"a".repeat(63)}.${"b".repeat(63)}.nycu.club`).length)
  ...
  it("normalizes overlapping grants to the broadest safe namespace", () => {
    expect(normalizeGrantSet(["www.magic.nycu.club", grant, grant, "photo.nycu.club"]))
      .toEqual([grant, "photo.nycu.club"]);
  });
  ...
  it("rejects apex, wildcard, external, and protected namespace grants", () => {
    ...
    expect(() => normalizeNamespaceGrant("api.nycu.club", "nycu.club", config.protectedHostnames))
      .toThrow(/保護/u);
  });
...
describe("record authorization and IDOR guard primitives", () => {
  ...
  it("keeps protected names, record IDs, and unsupported types unavailable to admin", () => {
    ...
    expect(() => assertRecordAccess(admin, {
      id: "protected-record-id",
      name: "magic.nycu.club",
      type: "A",
    }, config)).toThrow(/record ID/u);
    expect(() => assertRecordAccess(admin, {
      id: "safe-id",
      name: "magic.nycu.club",
      type: "NS",
    }, config)).toThrow(/白名單/u);
  });

  it("uses the server-fetched current record name for IDOR checks", () => {
    expect(() => assertRecordAccess(member, {
      id: "guessed-record-id",
      name: "photo.nycu.club",
      type: "A",
    }, config)).toThrow(/權限/u);
    expect(canSeeRecord(member, { name: "api.magic.nycu.club", type: "AAAA" }, config))
      .toBe(true);
    expect(canSeeRecord(member, { name: "photo.nycu.club", type: "AAAA" }, config))
      .toBe(false);
  });
});
