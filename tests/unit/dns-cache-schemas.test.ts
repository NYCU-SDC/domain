...
import {
  ...
  normalizePurgeUrls,
} from "../../app/lib/shared/cache";
import {
  normalizeDnsMutation,
  parseDnsRecordInput,
} from "../../app/lib/shared/dns/records";
...
describe("DNS discriminated record schemas", () => {
  ...
  it("rejects unsupported types even when the payload shape looks plausible", () => {
    expect(() => normalize({ content: "ns1.example.com", name: "@", namespace, ttl: 1, type: "NS" }))
      .toThrow(AppError);
  });

  it("forces DNS-only types at the schema and payload boundaries", () => {
    expect(() => normalize({ name: "@", namespace, priority: 10, proxied: true, target: "mail.example.com", ttl: 1, type: "MX" }))
      .toThrow(AppError);
  ...
  it("supports relative wildcard and underscore owner names without escaping namespace", () => {
    expect(normalize({ content: "verification", name: "_acme-challenge", namespace, ttl: 1, type: "TXT" }).hostname)
      .toBe("_acme-challenge.magic.nycu.club");
    expect(normalize({ content: "192.0.2.1", name: "*.dev", namespace, ttl: 1, type: "A" }).hostname)
      .toBe("*.dev.magic.nycu.club");
...
describe("cache purge validation and authorization", () => {
  ...
  it.each([
    "ftp://magic.nycu.club/file",
    "https://user:password@magic.nycu.club/private",
    "https://magic.nycu.club:99999/file",
    "not a url",
  ...
  it.each([
    "magic.nycu.club/../admin",
    "magic.nycu.club/%2e%2e/admin",
    "magic.nycu.club/%252e%252e/admin",
    "magic.nycu.club/assets\\..\\admin",
  ])("rejects prefix traversal %s", (prefix) => {
    expect(() => normalizePurgePrefixes({ prefixes: [prefix] })).toThrow(/traversal|encoding/u);
  });

  it("returns a normalized impact prefix", () => {
    expect(normalizePurgePrefixes({ prefixes: ["MAGIC.NYCU.CLUB/assets//images/"] }))
      .toEqual([{ hostname: namespace, prefix: `${namespace}/assets/images` }]);
  });
});
