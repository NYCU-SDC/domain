...
import {
  ...
  readGithubCallback,
  upsertGithubLogin,
} from "../../app/lib/server/auth/oauth.server";
import {
  createSession,
  getAuthenticatedSession,
  requireAdminSession,
  revokeCurrentSession,
} from "../../app/lib/server/auth/session.server";
import {
  createAdminUser,
  updateAdminUser,
} from "../../app/lib/server/admin/users.server";
...
const githubOAuthFetch: typeof fetch = vi.fn(async (input, init) => {
  ...
  if (url === "https://api.github.com/user") {
    expect(new Headers(init?.headers).get("Authorization"))
      .toBe("Bearer ephemeral-test-access-token");
...
describe("GitHub OAuth authorization code, state, and PKCE", () => {
  ...
  it("rejects expired OAuth temporary state", async () => {
    ...
    ).bind(crypto.randomUUID(), await sha256Hex(state), expiresAt - 600_000, expiresAt).run();
  ...
  it("sends the PKCE verifier and safely handles an upstream verifier rejection", async () => {
    ...
    await expect(exchangeGithubCode(
      "code",
      "wrong-verifier-value-with-sufficient-length-1234567890",
      env,
      rejectingFetch,
    )).rejects.toThrow(/authorization code/u);
  });
});
...
describe("numeric GitHub identity status and bootstrap behavior", () => {
  it("creates unknown users as pending without grants", async () => {
    ...
    const grants = await env.DB.prepare("SELECT namespace FROM namespace_grants WHERE user_id = ?")
  ...
  it("updates a renamed login while retaining the numeric identity", async () => {
    ...
    const row = await env.DB.prepare("SELECT github_login AS login FROM users WHERE github_id = ?")
      .bind(githubIdentity.id).first<{ login: string }>();
  ...
  it("activates immutable numeric bootstrap admins", async () => {
    ...
    const row = await env.DB.prepare("SELECT is_admin AS isAdmin, status FROM users WHERE github_id = '9001'")
      .first<{ isAdmin: number; status: string }>();
  ...
  it("preserves suspended status and does not create a general session during identity update", async () => {
    ...
    const sessionCount = await env.DB.prepare("SELECT COUNT(*) AS total FROM sessions")
      .first<{ total: number }>();
...
describe("opaque server-side sessions", () => {
  it("stores only a SHA-256 token hash and authenticates the original cookie", async () => {
    ...
    const row = await env.DB.prepare("SELECT token_hash AS tokenHash FROM sessions WHERE id = ?")
      .bind(created.sessionId).first<{ tokenHash: string }>();
    ...
    await expect(getAuthenticatedSession(authenticatedRequest(created.cookie), env))
      .resolves.toMatchObject({ id: created.sessionId, grants: ["magic.nycu.club"] });
  });
  ...
  it("rejects expired and revoked sessions", async () => {
    ...
    await env.DB.prepare("UPDATE sessions SET expires_at = ? WHERE id = ?")
      .bind(Date.now() - 1, expired.sessionId).run();
    ...
    await env.DB.prepare("UPDATE sessions SET revoked_at = ? WHERE id = ?")
      .bind(Date.now(), revoked.sessionId).run();
  ...
  it("revokes the current session on logout logic", async () => {
    ...
    await expect(revokeCurrentSession(authenticatedRequest(created.cookie), env))
      .resolves.toBe(created.sessionId);
  ...
  it("revokes a suspended user's session on the next authenticated request", async () => {
    ...
    await expect(getAuthenticatedSession(authenticatedRequest(created.cookie), env)).resolves.toBeNull();
    const row = await env.DB.prepare("SELECT revoked_at AS revokedAt FROM sessions WHERE id = ?")
      .bind(created.sessionId).first<{ revokedAt: number | null }>();
...
describe("admin safety and session revocation", () => {
  it("denies a regular user access to admin authorization", async () => {
    ...
    await addGrant(userId, "magic.nycu.club");
    const created = await createSession(userId, testRequest(), env);
    await expect(requireAdminSession(authenticatedRequest(created.cookie), env)).rejects
      .toThrow(/管理員/u);
  });
  ...
  it("prevents removal or suspension of the last active admin", async () => {
    ...
    await expect(updateAdminUser(adminId, { isAdmin: false }, testRequest(), env, actor, "req-last-admin"))
      .rejects.toThrow(/最後一位/u);
    await expect(updateAdminUser(adminId, { status: "suspended" }, testRequest(), env, actor, "req-last-admin-2"))
  ...
  it("revokes all target sessions when grants, note, status, or admin state changes", async () => {
    ...
    await updateAdminUser(
      targetId,
      { grants: ["magic.nycu.club", "www.magic.nycu.club"], note: "魔術社社長" },
      testRequest(),
      env,
      actor,
      "req-grant-update",
    );
    const sessionRow = await env.DB.prepare("SELECT revoked_at AS revokedAt FROM sessions WHERE id = ?")
      .bind(targetSession.sessionId).first<{ revokedAt: number | null }>();
    expect(sessionRow?.revokedAt).not.toBeNull();
    const grants = await env.DB.prepare("SELECT namespace FROM namespace_grants WHERE user_id = ? ORDER BY namespace")
      .bind(targetId).all<{ namespace: string }>();
  ...
  it("rejects duplicate GitHub numeric IDs when admin resolves by username", async () => {
    ...
    await expect(createAdminUser(
      { grants: [], isAdmin: false, note: null, status: "pending", username: "existing" },
      testRequest(),
      env,
...
describe("cache purge feature gates and rate limiting", () => {
  ...
  it("requires admin, feature flag, and exact destructive confirmation", () => {
    expect(() => assertPurgeEverythingAccess(sessionFor("member"), baseConfig, "PURGE nycu.club"))
      .toThrow(/admin/u);
    expect(() => assertPurgeEverythingAccess(sessionFor("admin", { isAdmin: true }), baseConfig, "PURGE nycu.club"))
      .toThrow(/未啟用/u);
    const enabled = { ...baseConfig, enablePurgeEverything: true };
    expect(() => assertPurgeEverythingAccess(sessionFor("admin", { isAdmin: true }), enabled, "purge"))
      .toThrow(/PURGE nycu.club/u);
    expect(() => assertPurgeEverythingAccess(sessionFor("admin", { isAdmin: true }), enabled, "PURGE nycu.club"))
      .not.toThrow();
  });
  ...
  it("maps a denied rate-limit binding to RATE_LIMITED", async () => {
    ...
    const error = await enforceRateLimit(deniedEnv, "cache", "user-1", "purge")
