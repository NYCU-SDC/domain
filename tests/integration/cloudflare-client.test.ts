...
describe("explicit Cloudflare API client", () => {
  ...
  it.each([
    ...
    const error = await client(fetcher).getDnsRecord(record.id).catch((caught: unknown) => caught);
  ...
  it("maps CNAME conflicts to a friendly message", async () => {
    ...
    await expect(client(fetcher).createDnsRecord({
  ...
  it("rejects malformed and non-JSON upstream responses", async () => {
    ...
    const nonJson: typeof fetch = async () => new Response("gateway html", {
  ...
  it("maps network failures and timeouts without leaking details", async () => {
    ...
    const error = await client(fetcher).getZone().catch((caught: unknown) => caught);
    expect(error).toBeInstanceOf(AppError);
  ...
  it("uses only explicit purge payloads", async () => {
    ...
    expect(bodies).toEqual([
      { files: ["https://magic.nycu.club/a"] },
      { hosts: ["magic.nycu.club"] },
      { prefixes: ["magic.nycu.club/assets"] },
