...
export function normalizeHostname(input: string, options: NormalizeHostnameOptions = {}): string {
	...
	const canonicalLabels = rawLabels.map((rawLabel, index) => {
		...
		const canonical = /[^\u0000-\u007f]/u.test(lower) ? normalizeIdnaLabel(lower) : lower;
		const valid = canonical.startsWith("_") ? options.allowUnderscore === true && underscoreLabelPattern.test(canonical) : asciiLabelPattern.test(canonical);
		if (!valid) {
...
export function isHostnameWithinNamespace(hostname: string, namespace: string): boolean {
	try {
		...
		return normalizedHostname === normalizedNamespace || normalizedHostname.endsWith(`.${normalizedNamespace}`);
	} catch {
		return false;
	}
}
...
export function normalizeNamespaceGrant(input: string, zoneName: string, protectedHostnames: ReadonlySet<string>): string {
