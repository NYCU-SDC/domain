...
export async function loader({ context, request }: Route.LoaderArgs) {
  ...
  const recent = await runtime.env.DB
    .prepare(
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Link className="button buttonPrimary" to="/dashboard/dns">管理 DNS Records</Link>}
      />
      {loaderData.upstreamError ? <div className={styles.error} role="alert"><Cloud /> <span>{loaderData.upstreamError}<small>Request ID: {loaderData.requestId}</small></span></div> : null}
      <section className={styles.metrics} aria-label="統計資料">
        <article className="card"><span><RadioTower /></span><div><small>可管理 namespace</small><strong>{loaderData.counts.namespaces}</strong></div></article>
        <article className="card"><span><Database /></span><div><small>DNS records</small><strong>{loaderData.counts.records}</strong></div></article>
        <article className="card"><span><Cloud /></span><div><small>Proxied records</small><strong>{loaderData.counts.proxied}</strong></div></article>
        <article className="card"><span><Activity /></span><div><small>最近 cache purge</small><strong className={styles.time}>{formatTime(loaderData.lastPurge)}</strong></div></article>
      </section>
      <div className={styles.columns}>
        <section className={`card ${styles.recent}`}>
          <div className={styles.sectionTop}><div><p className="eyebrow">AUDIT</p><h2>最近操作</h2></div><Link to="/dashboard/audit">查看全部</Link></div>
          {loaderData.recent.length ? (
            <ul>{loaderData.recent.map((event) => <li key={event.id}><span className="statusPill" data-tone={event.status === "success" ? "success" : event.status === "denied" ? "danger" : "warning"}>{event.status}</span><div><b>{event.action}</b><small>{event.hostname ?? event.namespace ?? "帳號操作"}</small></div><time>{formatTime(event.createdAt)}</time></li>)}</ul>
