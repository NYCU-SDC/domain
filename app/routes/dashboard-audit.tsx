...
export async function loader({ context, request }: Route.LoaderArgs) {
  ...
  const namespaceClause = namespaces.length
    ? `OR ((action LIKE 'dns.%' OR action LIKE 'cache.%') AND namespace IN (${namespaces.map(() => "?").join(",")}))`
    : "";
...
export default function DashboardAudit({ loaderData }: Route.ComponentProps) {
  return (
    <div>
      <PageHeader eyebrow="AUDIT" title="操作紀錄" description="你只能查看自己的操作，以及目前有權限 namespace 的 DNS／cache 事件。IP、token 與 admin note 不會顯示。" />
