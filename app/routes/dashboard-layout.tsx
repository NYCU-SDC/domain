      ...
      <div className={styles.shell}>
        <aside className={styles.sidebar} data-open={menuOpen}>
          <div className={styles.sidebarTop}>
            <NavLink className="brand" to="/dashboard"><span className="brandMark">N</span>nycu.club</NavLink>
            <button className={styles.closeMenu} onClick={() => setMenuOpen(false)} type="button" aria-label="關閉選單"><X /></button>
          </div>
          <nav className={styles.sideNav} aria-label="Dashboard 導覽">
            ...
            {loaderData.user.isAdmin ? (
              <>
                <p>Administration</p>
                <NavLink to="/admin" end className={({ isActive }) => (isActive ? styles.active : undefined)}><ShieldCheck />管理後台</NavLink>
                <NavLink to="/admin/users" className={({ isActive }) => (isActive ? styles.active : undefined)}><Users />使用者</NavLink>
              </>
            ) : null}
          </nav>
          <div className={styles.sideSecurity}><Activity /><div><b>安全連線</b><span>所有 mutation 皆經過 server-side authorization</span></div></div>
          <a className={styles.docsLink} href="/docs"><BookOpen /> 使用說明</a>
        </aside>
        {menuOpen ? <button className={styles.backdrop} onClick={() => setMenuOpen(false)} aria-label="關閉選單" type="button" /> : null}
        <div className={styles.workspace}>
          <header className={styles.topbar}>
            <button className={styles.menuButton} onClick={() => setMenuOpen(true)} type="button" aria-label="開啟選單"><Menu /></button>
            <label className={styles.namespaceSelect}>
              <span>Namespace</span>
              <select defaultValue="all" aria-label="選擇 namespace" name="namespace" onChange={(event) => {
                ...
                {loaderData.grants.map((grant) => <option key={grant} value={grant}>{grant}</option>)}
              </select>
            ...
            <div className={styles.headerActions}>
              ...
              <div className={styles.profile}>
                ...
                <span><b>@{loaderData.user.githubLogin}</b><small>{loaderData.user.isAdmin ? "Administrator" : "Member"}</small></span>
              </div>
              <Form action="/logout" method="post">
                <input type="hidden" name="csrfToken" value={loaderData.csrfToken} />
                <button className="button buttonGhost buttonSmall" aria-label="登出" title="登出" type="submit"><LogOut size={18} /></button>
              </Form>
            </div>
          </header>
...
export function HydrateFallback() {
  return <div className={styles.loadingShell}><div className="skeleton" /><div className="skeleton" /><div className="skeleton" /></div>;
}
