...
import { createAdminUser, getAdminUser, listAdminUsers, resolveGithubIdentityForAdmin, updateAdminUser } from "~/features/admin/server/users.server";
import { prepareAuditStatement, writeAuditLog, writeFailureAudit } from "~/features/audit/server/audit.server";
import { getAuthenticatedSession, requireActiveSession, requireAdminSession, requireAuthenticatedSession } from "~/features/auth/server/session.server";
import { assertHostnameAccess, assertPurgeEverythingAccess, assertRecordAccess, canSeeRecord, isProtectedHostname, matchingNamespace } from "~/features/dns/server/authorization.server";
import { CloudflareClient } from "~/server/cloudflare/client.server";
import { getAppConfig, requireSecret } from "~/server/config.server";
import { createDb } from "~/server/db/client.server";
import { auditLogs, sessions, users } from "~/server/db/schema.server";
import { getWorkerRuntime } from "~/server/runtime.server";
import { enforceRateLimit, type RateLimitKind } from "~/server/security/rate-limit.server";
import { assertCsrfToken, assertSameOrigin, createCsrfToken } from "~/server/security/request.server";
import { adminUsersQuerySchema, createUserSchema, resolveGithubUserSchema, updateUserSchema } from "~/shared/lib/admin";
import { apiFailure, apiSuccess, assertJsonRequest } from "~/shared/lib/api";
import { normalizePurgeHostnames, normalizePurgePrefixes, normalizePurgeUrls, purgeEverythingSchema } from "~/shared/lib/cache";
import { normalizeHostname } from "~/shared/lib/dns/hostname";
import { allowedDnsTypes, normalizeDnsMutation, parseDnsRecordInput } from "~/shared/lib/dns/records";
...
function publicRecord(record: Awaited<ReturnType<CloudflareClient["getDnsRecord"]>>, session: Awaited<ReturnType<typeof requireActiveSession>>, env: Env) {
	const config = getAppConfig(env);
	const namespace = session.user.isAdmin ? null : matchingNamespace(record.name, session.grants);
	return {
...
async function secureMutation(request: Request, env: Env, kind: RateLimitKind, options: { admin?: boolean; action: string }) {
	const session = options.admin ? await requireAdminSession(request, env) : await requireActiveSession(request, env);
	assertSameOrigin(request, env);
	await assertCsrfToken(request.headers.get("X-CSRF-Token"), session.id, session.user.id, env);
	await enforceRateLimit(env, kind, session.user.id, options.action);
	return session;
}

async function getAuditPage(request: Request, env: Env, admin: boolean): Promise<unknown> {
	const session = admin ? await requireAdminSession(request, env) : await requireActiveSession(request, env);
	...
	if (filters.actor && admin) {
		conditions.push(or(eq(auditLogs.actorUserId, filters.actor), like(users.githubLogin, `%${filters.actor}%`)));
	}
	if (!admin) {
		const namespaceCondition = session.grants.length > 0 ? and(inArray(auditLogs.namespace, session.grants), or(like(auditLogs.action, "dns.%"), like(auditLogs.action, "cache.%"))) : undefined;
		conditions.push(namespaceCondition ? or(eq(auditLogs.actorUserId, session.user.id), namespaceCondition) : eq(auditLogs.actorUserId, session.user.id));
	}
...
async function dispatchGet(request: Request, env: Env, requestId: string): Promise<Response> {
	...
	if (path === "/dns-records") {
		...
		const records = (await cloudflareClient(env, requestId).listDnsRecords())
			...
			.filter(record => filters.proxied === "all" || record.proxied === (filters.proxied === "yes"))
			.filter(record => !filters.namespace || record.name === filters.namespace || record.name.endsWith(`.${filters.namespace}`))
			.filter(record => {
...
async function dispatchMutation(request: Request, env: Env, requestId: string, audit: MutationAuditContext): Promise<Response> {
	const path = getPath(request);
	...
	if (path === "/sessions/revoke-others" && method === "POST") {
		...
		const results = await env.DB.batch([env.DB.prepare("UPDATE sessions SET revoked_at = ? WHERE user_id = ? AND id <> ? AND revoked_at IS NULL").bind(now, session.user.id, session.id), event]);
	...
	if (path === "/dns-records" && method === "POST") {
		...
		audit.actorUserId = session.user.id;
		const config = getAppConfig(env);
		const normalized = normalizeDnsMutation(parseDnsRecordInput(await readJsonBody(request)), config.zoneName, config.allowProxiedDeepSubdomains);
	...
	if (path.startsWith("/cache/purge/") && method === "POST") {
		...
		const session = await secureMutation(request, env, isEverything ? "purge-everything" : "cache", { action: path });
		audit.actorUserId = session.user.id;
		...
		if (path === "/cache/purge/urls") {
			...
			const namespaces = prefixes.map(prefix => assertHostnameAccess(session, prefix.hostname, config));
	...
	if (revokeMatch?.[1] && method === "POST") {
		...
		const result = await env.DB.batch([env.DB.prepare("UPDATE sessions SET revoked_at = ? WHERE user_id = ? AND revoked_at IS NULL").bind(now, userId), log]);
...
export async function loader({ context, request }: Route.LoaderArgs): Promise<Response> {
	...
	if (request.method !== "GET") {
		return apiFailure(new AppError("VALIDATION_ERROR", "此 endpoint 不支援該 HTTP method", { httpStatus: 405 }), requestId);
	}
	try {
...
export async function action({ context, request }: Route.ActionArgs): Promise<Response> {
	...
	if (!actionName) {
		return apiFailure(new AppError("NOT_FOUND", "找不到此 API endpoint 或 HTTP method"), requestId);
	}
	...
	try {
		...
		await writeFailureAudit(env.DB, request, env, { ...audit, requestId }, error);
		return apiFailure(error, requestId);
	}
}
