...
import { getClientIp, summarizeUserAgent } from "~/server/security/request.server";
import { hmacSha256Hex } from "~/shared/lib/crypto";
...
function sanitizeSnapshot(value: unknown, depth = 0): unknown {
	...
	if (typeof value === "object") {
		...
		for (const [key, item] of Object.entries(value)) {
			sanitized[key] = sensitiveKeyPattern.test(key) ? "[redacted]" : sanitizeSnapshot(item, depth + 1);
		}
		return sanitized;
	}
...
export async function writeAuditLog(database: D1Database, request: Request, env: Env, event: AuditEvent): Promise<void> {
	const statement = await prepareAuditStatement(database, request, env, event);
	await statement.run();
}

export async function prepareAuditStatement(database: D1Database, request: Request, env: Env, event: AuditEvent): Promise<D1PreparedStatement> {
	const now = Date.now();
	const ipHash = await hmacSha256Hex(requireSecret(env, "IP_HASH_SECRET"), getClientIp(request));
	return database
		.prepare(
...
export function auditStatusForError(error: unknown): AuditStatus {
	...
	return appError.code === "FORBIDDEN" || appError.code === "PROTECTED_RESOURCE" || appError.code === "UNAUTHENTICATED" ? "denied" : "error";
}

export async function writeFailureAudit(database: D1Database, request: Request, env: Env, base: Omit<AuditEvent, "errorCode" | "errorMessage" | "status">, error: unknown): Promise<void> {
	const appError = toAppError(error);
	try {
