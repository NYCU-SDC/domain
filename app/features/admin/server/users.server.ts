...
import type { AuditEvent } from "~/features/audit/server/audit.server";
...
import { users } from "~/server/db/schema.server";
import { resolvePublicGithubUser, type GithubIdentity } from "~/server/github/client.server";
import type { CreateUserInput, UpdateUserInput } from "~/shared/lib/admin";
import { normalizeGrantSet, normalizeNamespaceGrant } from "~/shared/lib/dns/hostname";
...
async function getGrantsByUserIds(database: D1Database, userIds: readonly string[]): Promise<Map<string, string[]>> {
...
export async function getAdminUser(database: D1Database, env: Env, userId: string): Promise<AdminUserView> {
...
export async function listAdminUsers(
	...
	if (filters.search) {
		...
		conditions.push(or(like(users.githubLogin, query), like(users.githubId, query), like(users.note, query)));
	}
	...
	const grants = await getGrantsByUserIds(
		database,
...
function normalizeRequestedGrants(grants: readonly string[], env: Env): string[] {
	...
	return normalizeGrantSet(grants.map(grant => normalizeNamespaceGrant(grant, config.zoneName, config.protectedHostnames)));
}
...
async function auditStatementsForUserUpdate(
	...
	return Promise.all(events.map(event => prepareAuditStatement(database, request, env, event)));
}

export async function createAdminUser(input: CreateUserInput, request: Request, env: Env, actor: AuthenticatedSession, requestId: string, fetcher?: typeof fetch): Promise<AdminUserView> {
	const identity = await resolvePublicGithubUser(input.username, fetcher);
	const existing = await env.DB.prepare("SELECT id FROM users WHERE github_id = ?").bind(identity.id).first<{ id: string }>();
	if (existing) {
	...
	const statements: D1PreparedStatement[] = [
		...
		).bind(userId, identity.id, identity.login, identity.name, identity.avatarUrl, identity.profileUrl, status, isAdmin ? 1 : 0, input.note, now, now),
		...grants.map(grant =>
			env.DB.prepare("INSERT INTO namespace_grants (id, user_id, namespace, created_by_user_id, created_at) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), userId, grant, actor.user.id, now)
		),
...
export async function updateAdminUser(userId: string, input: UpdateUserInput, request: Request, env: Env, actor: AuthenticatedSession, requestId: string): Promise<AdminUserView> {
	...
	if (before.isAdmin && before.status === "active" && (!after.isAdmin || after.status !== "active")) {
	...
	const now = Date.now();
	const statements: D1PreparedStatement[] = [
		env.DB.prepare("UPDATE users SET status = ?, is_admin = ?, note = ?, updated_at = ? WHERE id = ?").bind(after.status, after.isAdmin ? 1 : 0, after.note, now, userId),
		...
		...(await auditStatementsForUserUpdate(env.DB, request, env, actor, requestId, before, after))
	];
	await env.DB.batch(statements);
	return after;
}

export function resolveGithubIdentityForAdmin(username: string, fetcher?: typeof fetch): Promise<GithubIdentity> {
