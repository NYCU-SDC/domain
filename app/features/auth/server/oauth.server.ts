...
import { getAuthenticatedGithubIdentity, type GithubIdentity } from "~/server/github/client.server";
import { parseCookies } from "~/server/security/request.server";
import { constantTimeEqual, decryptJson, encryptJson, randomToken, sha256Base64Url, sha256Hex } from "~/shared/lib/crypto";
...
export async function createGithubAuthorization(request: Request, env: Env): Promise<{ authorizationUrl: string; cookie: string }> {
	...
	const encryptedCookie = await encryptJson({ expiresAt, state, verifier }, requireSecret(env, "AUTH_SECRET"));
...
async function consumeOauthState(request: Request, state: string, env: Env): Promise<string> {
	...
	try {
		parsed = oauthCookieSchema.parse(await decryptJson(encrypted, requireSecret(env, "AUTH_SECRET")));
	} catch (error) {
...
export async function exchangeGithubCode(code: string, verifier: string, env: Env, fetcher: typeof fetch = fetch): Promise<GithubIdentity> {
	const config = getAppConfig(env);
...
export async function readGithubCallback(request: Request, env: Env, fetcher?: typeof fetch): Promise<GithubIdentity> {
...
export async function upsertGithubLogin(identity: GithubIdentity, env: Env): Promise<GithubLoginResult> {
	const config = getAppConfig(env);
	const isBootstrapAdmin = config.bootstrapAdminGithubIds.has(identity.id);
	const db = createDb(env.DB);
	const existing = await db.select({ id: users.id, isAdmin: users.isAdmin, status: users.status }).from(users).where(eq(users.githubId, identity.id)).limit(1);
	const now = Date.now();
	...
	if (!userId) {
		...
		try {
			...
			const raced = await db.select({ id: users.id, status: users.status }).from(users).where(eq(users.githubId, identity.id)).limit(1);
