...
function mapCloudflareError(
  status: number,
  errors: ReadonlyArray<z.infer<typeof cloudflareErrorSchema>>,
): AppError {
  ...
  if (
    status === 409 ||
    upstreamMessage.includes("already exists") ||
    upstreamMessage.includes("cname")
  ) {
    const message = upstreamMessage.includes("cname")
      ? "CNAME 無法與同名的 A、AAAA 或其他 CNAME record 並存"
      : "相同的 DNS record 已存在，或與現有設定衝突";
    return new AppError("CONFLICT", message);
  }
  if (status === 400 || status === 422) {
    return new AppError(
      "VALIDATION_ERROR",
      safeCode ? `Cloudflare 拒絕了 DNS 資料（code ${safeCode}）` : "Cloudflare 拒絕了 DNS 資料",
    );
  }
  return new AppError(
    "UPSTREAM_ERROR",
    safeCode ? `Cloudflare API 暫時無法完成操作（code ${safeCode}）` : "Cloudflare API 暫時無法完成操作",
  );
}
  ...
  async listDnsRecords(): Promise<CloudflareDnsRecord[]> {
    ...
    while (true) {
      const response = await this.#fetcher(
        `${API_BASE_URL}/zones/${this.#zoneId}/dns_records?page=${page}&per_page=5000&order=name&direction=asc`,
        {
          headers: {
