import { CloudflareClient, type CloudflareDnsRecord } from "../cloudflare/client.server";
import { getAppConfig, requireSecret } from "../config.server";
import {
  canSeeRecord,
  isProtectedHostname,
  matchingNamespace,
} from "../permissions/dns-authorization.server";
...
export function toDnsRecordView(
  ...
  return {
    ...
    name: record.name,
    namespace: session.user.isAdmin
      ? null
...
export async function listAuthorizedDnsRecords(
  session: AuthenticatedSession,
  env: Env,
  requestId: string,
  ...
  return (await client.listDnsRecords())
    .filter((record) => canSeeRecord(session, record, config))
    .map((record) => toDnsRecordView(record, session, env));
}
