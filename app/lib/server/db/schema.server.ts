...
export const users = sqliteTable(
	...
	table => [uniqueIndex("users_github_id_uq").on(table.githubId), index("users_status_idx").on(table.status), index("users_login_idx").on(table.githubLogin)]
);
...
export const namespaceGrants = sqliteTable(
	...
	table => [
		uniqueIndex("namespace_grants_user_namespace_uq").on(table.userId, table.namespace),
...
export const oauthStates = sqliteTable(
	...
	table => [uniqueIndex("oauth_states_state_hash_uq").on(table.stateHash), index("oauth_states_expiry_idx").on(table.expiresAt)]
);
...
export const accessApplications = sqliteTable(
	"access_applications",
	{
		id: text("id").primaryKey(),
		...
		notificationAttemptedAt: integer("notification_attempted_at"),
	...
	table => [
		index("access_applications_status_idx").on(table.status),
		index("access_applications_namespace_idx").on(table.requestedNamespace),
		index("access_applications_github_login_idx").on(table.githubLogin),
		index("access_applications_created_idx").on(table.createdAt)
	]
);

export const userRelations = relations(users, ({ many }) => ({
	reviewedApplications: many(accessApplications),
	grants: many(namespaceGrants),
	sessions: many(sessions)
}));

export const accessApplicationRelations = relations(accessApplications, ({ one }) => ({
	reviewedBy: one(users, {
		fields: [accessApplications.reviewedByUserId],
		references: [users.id]
	})
}));
