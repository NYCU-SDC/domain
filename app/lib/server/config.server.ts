...
export function getAppConfig(env: Env): AppConfig {
  ...
  const protectedHostnames = new Set(
    parseJsonArray(env.PROTECTED_HOSTNAMES, "PROTECTED_HOSTNAMES").map((hostname) =>
      normalizeHostname(hostname, { allowUnderscore: true, allowWildcard: false }),
    ),
  );
  const protectedRecordIds = new Set(
    parseJsonArray(env.PROTECTED_RECORD_IDS, "PROTECTED_RECORD_IDS"),
  );
  ...
  if (bootstrapIds.some((id) => !/^\d+$/u.test(id))) {
    throw new AppError(
      "INTERNAL_ERROR",
      "BOOTSTRAP_ADMIN_GITHUB_IDS 只能包含逗號分隔的 GitHub numeric IDs",
    );
  }
  const sessionMaxAgeSeconds = Number(env.SESSION_MAX_AGE_SECONDS);
  if (
    !Number.isSafeInteger(sessionMaxAgeSeconds) ||
  ...
  return {
    allowProxiedDeepSubdomains: parseBoolean(
      env.ALLOW_PROXIED_DEEP_SUBDOMAINS,
      "ALLOW_PROXIED_DEEP_SUBDOMAINS",
    ),
    appOrigin,
    bootstrapAdminGithubIds: new Set(bootstrapIds),
    enablePurgeEverything: parseBoolean(
      env.ENABLE_PURGE_EVERYTHING,
      "ENABLE_PURGE_EVERYTHING",
    ),
