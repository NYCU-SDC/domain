import { allowedDnsTypes, type AllowedDnsType } from "../../shared/dns/records";
import {
...
export function matchingNamespace(
  hostname: string,
...
export function assertHostnameAccess(
...
export function assertRecordAccess(
  session: AuthenticatedSession,
  record: RecordAuthorizationTarget,
  config: AppConfig,
): string | null {
  ...
  if (
    record.type &&
    !allowedDnsTypes.includes(record.type.toUpperCase() as AllowedDnsType)
  ) {
...
export function canSeeRecord(
  ...
  try {
    ...
    return (
      ...
      session.grants.some((grant) => isHostnameWithinNamespace(canonical, grant))
    );
  } catch {
    return false;
  }
...
export function assertPurgeEverythingAccess(
  session: AuthenticatedSession,
  config: AppConfig,
  confirmation: string,
): void {
