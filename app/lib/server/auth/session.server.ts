...
import { namespaceGrants, sessions, users } from "../db/schema.server";
import {
  clearSessionCookie,
  hashClientIp,
  hashUserAgent,
...
export async function getAuthenticatedSession(
  ...
  const grants = await db
    .select({ namespace: namespaceGrants.namespace })
    .from(namespaceGrants)
    .where(eq(namespaceGrants.userId, row.userId));
  return {
...
export async function createSession(
  ...
  await env.DB
    ...
    .bind(
      ...
      now + maxAge * 1000,
      now,
      await hashClientIp(request, env),
      await hashUserAgent(request),
    )
    .run();
