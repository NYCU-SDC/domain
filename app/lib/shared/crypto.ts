...
export async function constantTimeEqual(left: string, right: string): Promise<boolean> {
  ...
  return (crypto.subtle as TimingSafeSubtleCrypto).timingSafeEqual(
    ownedBuffer(leftHash),
    ownedBuffer(rightHash),
  );
}

async function deriveAesKey(secret: string): Promise<CryptoKey> {
  const keyMaterial = await sha256Bytes(`nycu.club oauth cookie:${secret}`);
  return crypto.subtle.importKey("raw", ownedBuffer(keyMaterial), "AES-GCM", false, [
    "decrypt",
    "encrypt",
  ]);
}
...
export async function decryptJson(value: string, secret: string): Promise<unknown> {
  ...
  const plaintext = await crypto.subtle.decrypt(
    { iv: ownedBuffer(decodeBase64Url(ivValue)), name: "AES-GCM" },
    key,
    ownedBuffer(decodeBase64Url(ciphertextValue)),
  );
