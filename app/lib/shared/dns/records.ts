...
export function normalizeDnsMutation(
  input: DnsRecordInput,
  zoneName: string,
  allowProxiedDeepSubdomains: boolean,
): NormalizedDnsMutation {
  ...
  const proxied = input.type === "A" || input.type === "AAAA" || input.type === "CNAME"
    ? input.proxied
    : false;
  ...
  if (proxied && isDeepSubdomain(baseHostname, zoneName) && !allowProxiedDeepSubdomains) {
    throw new AppError(
      "VALIDATION_ERROR",
      "此 hostname 是多層子網域，系統目前禁止開啟 Proxy；請改用 DNS only 或聯絡管理員確認憑證設定",
    );
  }
